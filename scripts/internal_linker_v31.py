...
def notify_webhook(url, payload, timeout=8):
    """POST a compact JSON payload to the n8n webhook. Best-effort: never
    raises — a failed notification must not fail the linker run itself."""
    if not url:
        return
    try:
        req = request.Request(url, data=json.dumps(payload).encode(),
            headers={"Content-Type": "application/json",
                     # your n8n host-hosted n8n WAF blocks non-browser user agents (403)
                     "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 "
                                   "(KHTML, like Gecko) Chrome/126.0 Safari/537.36"})
        with request.urlopen(req, timeout=timeout) as resp:
            resp.read()
        print(f"  ↪ notify sent → {url} ({payload.get('type', '?')})")
...
def main():
    ...
    ap.add_argument("--notify", default="", metavar="URL",
                    help="n8n webhook URL: POST a compact run summary (run_report / "
                         "review_ready / run_error) after the run. Best-effort, never fatal.")
    args = ap.parse_args()
    ...
    tier_counts = {"inline": 0, "mid": 0, "footer": 0}
    backups = []
    ...
    # ---- Notify n8n webhook (v3.1: Lever 2+3, best-effort, never fatal)
    if args.notify:
        import datetime
        ...
        if args.dry_run and args.export_diffs:
            ...
            notify_webhook(args.notify, {
                "type": "review_ready",
                "site": os.environ.get("WP_SITE_URL", "").replace("https://", ""),
                "timestamp": ts,
                "new_diffs": len(diffs),
                "top": [{"from": d[0].rstrip("/").split("/")[-1][:30],
                         "to": d[1].rstrip("/").split("/")[-1][:30],
                         "score": d[2], "tier": "mid/footer" if d[3] == 3 else "inline"}
                        for d in diffs[:3]],
            })
        elif args.inline:
            # linked posts = sources with at least one DONE row in the merged sheet
            linked_sources = set()
            if not args.dry_run:
                for r in rows[1:]:
                    if (r[-1] or "").strip() == "DONE":
                        linked_sources.add(r[0])
            pending = max(0, len(cand) - (inserted + dupes + failed + skipped))
            notify_webhook(args.notify, {
                "type": "run_report",
                "site": os.environ.get("WP_SITE_URL", "").replace("https://", ""),
                "mode": "dry_run" if args.dry_run else "bypass",
                "timestamp": ts,
                "results": {"inserted": inserted, "duplicate": dupes,
                            "skipped": skipped, "failed": failed},
                "tiers": tier_counts,
                "coverage": {"posts": len(arts),
                             "linked": len(linked_sources) if not args.dry_run else None,
                             "pending": pending},
            })
...
if __name__ == "__main__":
    try:
        ...
        # fatal/unexpected error: notify then re-raise (exit code preserved)
        notify_url = ""
        try:
            if "--notify" in sys.argv:
                notify_url = sys.argv[sys.argv.index("--notify") + 1]
        except IndexError:
            pass
        if notify_url:
            notify_webhook(notify_url, {
                "type": "run_error",
                "site": os.environ.get("WP_SITE_URL", "").replace("https://", ""),
                "timestamp": __import__("datetime").datetime.now().strftime("%Y-%m-%d %H:%M WIB"),
                "stage": "main",
                "error": str(e)[:200],
